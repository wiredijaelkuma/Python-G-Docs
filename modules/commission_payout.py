"""
Commission Analysis with Payout Section
"""
...
def render_commission_analysis(df, COLORS, HEAT_COLORS):
    """Complete commission analysis with payout tracking"""
    ...
    # Filter cleared payments in the selected period
    if 'CLEARED_DATE' in commission_df.columns and 'CATEGORY' in commission_df.columns:
        cutoff_date = commission_df['CLEARED_DATE'].max() - timedelta(days=payout_days)
        cleared_payments = commission_df[
            (commission_df['CATEGORY'] == 'CLEARED') & 
            (commission_df['CLEARED_DATE'] >= cutoff_date) &
        ...
        if not cleared_payments.empty:
            ...
            with col3:
                if 'TRANSACTION_ID' in cleared_payments.columns:
                    unique_transactions = cleared_payments['TRANSACTION_ID'].nunique()
                else:
                    unique_transactions = len(cleared_payments)
            ...
            if 'AGENT' in cleared_payments.columns:
                ...
                if 'TRANSACTION_ID' in cleared_payments.columns:
                    payout_summary = cleared_payments.groupby('AGENT').agg({
                        ...
                        'CLEARED_DATE': ['min', 'max']
                    })
                    payout_summary.columns = ['Total_Records', 'Unique_Transactions', 'First_Cleared', 'Last_Cleared']
                else:
                    payout_summary = cleared_payments.groupby('AGENT').agg({
                        'AGENT': 'count',
                        'CLEARED_DATE': ['min', 'max']
                    })
                    payout_summary.columns = ['Unique_Transactions', 'First_Cleared', 'Last_Cleared']
                    payout_summary['Total_Records'] = payout_summary['Unique_Transactions']
                
                # Format dates
                payout_summary['First_Cleared'] = payout_summary['First_Cleared'].dt.strftime('%Y-%m-%d')
                payout_summary['Last_Cleared'] = payout_summary['Last_Cleared'].dt.strftime('%Y-%m-%d')
                ...
                with st.expander("📄 Detailed Transaction List for Payout"):
                    if 'TRANSACTION_ID' in cleared_payments.columns:
                        transaction_details = cleared_payments[['AGENT', 'TRANSACTION_ID', 'CLEARED_DATE']].copy()
                        transaction_details['CLEARED_DATE'] = transaction_details['CLEARED_DATE'].dt.strftime('%Y-%m-%d')
                        transaction_details = transaction_details.sort_values(['AGENT', 'CLEARED_DATE'])
                        ...
                        st.info("Transaction ID column not available for detailed breakdown")
            else:
                st.warning("Agent information not available")
        else:
            st.info(f"No cleared payments found in the last {clearance_days}")
    else:
        st.warning("Cleared date or category information not available")
