"""
Enhanced Hybrid Agent Analysis Module - 6 Displays Per Subtab
"""
...
def render_agent_hybrid_analysis(df, COLORS, HEAT_COLORS):
    """Complete hybrid agent analysis with 6 displays per subtab"""
...
def render_commission_analysis_enhanced(agent_commission, selected_agent, COLORS, HEAT_COLORS):
    ...
    with col2:
        ...
        if 'PROCESSED_DATE' in agent_commission.columns and 'CLEARED_DATE' in agent_commission.columns:
            # Calculate processing time for cleared payments
            cleared_payments = agent_commission[
                (agent_commission['CATEGORY'] == 'CLEARED') & 
                agent_commission['PROCESSED_DATE'].notna() & 
                agent_commission['CLEARED_DATE'].notna()
            ].copy()
            ...
            if not cleared_payments.empty:
                ...
                st.info("No cleared payment timeline data available")
        else:
            # Alternative chart - weekly payment volume
            if 'PROCESSED_DATE' in agent_commission.columns:
