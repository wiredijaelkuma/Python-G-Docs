...
LOGO_IMAGE = "assets/pepe-dashboard.png"
...
# Safely get the date and format it
    ...
    pdf_content_bytes = pdf.output(dest='S').encode('latin1') # 'latin1' is a common encoding for FPDF output
...
with tab1:
    ...
    with col1:
        ...
        if 'AGENT' in df.columns:
            ...
            # --- Start of Weekly Active Sales for Top Performers ---
        ...
        if 'ENROLLED_DATE' in df.columns and 'AGENT' in df.columns:
            ...
            print(f"Active df weekly head (after 'ACTIVE' filter):\n{active_df_weekly.head()}")
            print(f"Max ENROLLED_DATE in active_df_weekly: {active_df_weekly['ENROLLED_DATE'].max()}")
            # --- DEBUG END ---

            # Group by week and agent (W-MON means week ends on Monday)
            weekly_agent_performance = active_df_weekly.groupby([
                pd.Grouper(key='ENROLLED_DATE', freq='W-MON'), 'AGENT'
            ]).size().reset_index(name='Active_Contracts')
            
            # --- DEBUG START ---
            print(f"Weekly agent performance head:\n{weekly_agent_performance.head()}")
            print(f"Weekly agent performance tail:\n{weekly_agent_performance.tail()}")
            print(f"Unique ENROLLED_DATEs (Mondays) in weekly_agent_performance: {sorted(weekly_agent_performance['ENROLLED_DATE'].unique(), reverse=True)}")
            # --- DEBUG END ---
            
            # Create a user-friendly week string (e.g., "YYYY-MM-DD - Jamboree-MM-DD")
            weekly_agent_performance['WEEK_DISPLAY'] = weekly_agent_performance['ENROLLED_DATE'].dt.strftime('%Y-%m-%d') + \
                                                      ' - ' + (weekly_agent_performance['ENROLLED_DATE'] + pd.to_timedelta('6 days')).dt.strftime('%Y-%m-%d')
            
            # Get all unique display weeks for the dropdown, sorted reverse chronologically
            all_weeks_display = sorted(weekly_agent_performance['WEEK_DISPLAY'].unique(), reverse=True)
            
            # --- DEBUG START ---
            print(f"All weeks for display (dropdown options): {all_weeks_display}")
            # --- DEBUG END ---

            if all_weeks_display:
                # Allow user to select a week
                selected_week_display = st.selectbox("Select Week:", all_weeks_display, key="weekly_sales_selectbox")
                
                # --- DEBUG START ---
                print(f"User selected week display: {selected_week_display}")
                # --- DEBUG END ---

                # Extract the start date (Monday) from the selected display string to filter the DataFrame
                selected_week_start_date_str = selected_week_display.split(' - ')[0]
                
                # --- DEBUG START ---
                print(f"Extracted start date string for filtering: {selected_week_start_date_str}")
                # --- DEBUG END ---

                # Filter data for the selected week (using the original ENROLLED_DATE, which is the Monday)
                # Need to convert the original ENROLLED_DATE back to string for comparison
                selected_week_data = weekly_agent_performance[
                    weekly_agent_performance['ENROLLED_DATE'].dt.strftime('%Y-%m-%d') == selected_week_start_date_str
                ]
                
                # --- DEBUG START ---
                print(f"Selected week data for graph (before top N filtering):\n{selected_week_data}")
                # --- DEBUG END ---
                
                # Sort by Active_Contracts to ensure consistent top performers for the selected week
                selected_week_data = selected_week_data.sort_values(by='Active_Contracts', ascending=False)
                
                # Determine top performers for the selected week (e.g., top 10)
                top_agents_selected_week = selected_week_data.head(10) # Adjust head(N) as needed
                
                # --- DEBUG START ---
                print(f"Top agents for selected week:\n{top_agents_selected_week}")
                # --- DEBUG END ---
                ...
                if not top_agents_selected_week.empty:
                    fig = px.bar(
                        ...
                        x='AGENT',
                        y='Active_Contracts',
                        title=f"Top 10 Active Contracts for Week of {selected_week_display}",
        ...
        # --- End of Weekly Active Sales for Top Performers ---
    ...
    if 'ENROLLED_DATE' in df.columns:
        ...
        with col2:
            ...
            if 'WEEK' in df.columns:
                weekly_data = df.groupby(['WEEK', 'CATEGORY']).size().unstack(fill_value=0).reset_index()
                weekly_data['Success_Rate'] = weekly_data.get('ACTIVE', 0) / weekly_data.sum(axis=1) * 100
                ...
                fig = px.line(
                    ...
                    labels={'WEEK': 'Week Number', 'Success_Rate': 'Success Rate (%)'}
                )
                st.plotly_chart(fig, use_container_width=True, key="weekly_success_rate_chart")
            else:
                st.warning("Week data not available")
...
# --- Footer ---
st.markdown("---")
st.caption("© 2023 Pepe's Power Solutions | Dashboard v2.1 | Data updated daily")
