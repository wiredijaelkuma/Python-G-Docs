...
# --- Status Configuration (CRITICAL: Review and update these lists carefully!) ---
# These statuses (uppercase, stripped) are considered an active sale/enrollment.
# These will be the numerator for stick rate and primary sales counts.
# UPDATED based on user feedback and screenshot.
ACTIVE_SALE_STATUSES = ["ACTIVE"] 

# These statuses (uppercase, stripped) are problematic/potential churn but tracked separately.
# For stick rate calculation, these will be counted as "non-stick" because they are NOT in ACTIVE_SALE_STATUSES.
POTENTIAL_RESCUE_STATUSES = ["NSF"]

# Any other status not in ACTIVE_SALE_STATUSES will also be considered "non-stick" for stick rate.
# If you have specific statuses that are definite "hard cancellations", you can list them for separate tracking,
# but they will already be part of the "non-stick" group for stick rate if not in ACTIVE_SALE_STATUSES.
# HARD_CANCELLED_STATUSES = ["CANCELLED", "TERMINATED", "PENDING CANCELLATION"] # Example for separate counting if needed
    ...
    if EXPECTED_COL_ENROLLED_DATE in df.columns:
        if not pd.api.types.is_datetime64_any_dtype(df[EXPECTED_COL_ENROLLED_DATE]):
            ...
            if df[EXPECTED_COL_ENROLLED_DATE].isnull().all() and not df.empty:
                print(f"    WARNING (standardize_df): All '{EXPECTED_COL_ENROLLED_DATE}' became NaT for '{sheet_name_for_logging}'.")
    
    if EXPECTED_COL_STATUS in df.columns:
        df[EXPECTED_COL_STATUS] = df[EXPECTED_COL_STATUS].astype(str).str.strip().str.upper()
    # else:
        # print(f"    WARNING (standardize_df): Col '{EXPECTED_COL_STATUS}' not found in '{sheet_name_for_logging}'.")
    return df
...
def fetch_and_combine_data(gspread_client, spreadsheet_title, raw_sheet_names_list):
    ...
    try:
        ...
        if dfs_to_concat:
            ...
            # STATUS column already standardized from individual DFs
...
# --- Visualization Creation Functions (largely unchanged) ---
def create_bar_chart(df, title, x_col, y_col, color_col=None, color_map=None):
    ...
    fig.update_layout(font_family="Arial", title_font_size=20, title_x=0.5, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', margin=dict(l=20,r=20,t=60,b=20), xaxis_title=None, yaxis_title=None, hovermode="x unified")
...
def create_gauge_chart(value, min_val, max_val, title):
    fig = go.Figure(go.Indicator(mode="gauge+number", value=value, domain={'x': [0, 1], 'y': [0, 1]}, title={'text': title, 'font': {'size': 20}},
        gauge={'axis': {'range': [min_val, max_val]}, 'bar': {'color': "darkblue"},
               'steps': [{'range': [0, min_val * 0.999], 'color': "lightgray"}, {'range': [min_val, max_val*0.6], 'color': "orange"},
                         {'range': [max_val*0.6, max_val*0.8], 'color': "yellow"}, {'range': [max_val*0.8, max_val], 'color': "green"}],
...
def create_heatmap_chart(df, title_prefix, date_col='Date', value_col='Daily Active Enrollments'):
    ...
    if not pd.api.types.is_datetime64_any_dtype(df[date_col]):
        ...
        if df[date_col].isnull().all(): return go.Figure().update_layout(title_text=f"{title_prefix} Heatmap (Date column invalid)")
    df_copy = df.copy()
    ...
    fig = go.Figure(data=go.Heatmap(z=heatmap_pivot.values, x=heatmap_pivot.columns, y=heatmap_pivot.index, colorscale='Viridis', hoverongaps=False, text=heatmap_pivot.values, hovertemplate='<b>%{y}</b>, Week %{x}<br>Enrollments: %{z}<extra></extra>'))
    fig.update_layout(title=title, title_x=0.5, xaxis_title="ISO Week of Year", yaxis_title="Day of Week", font_family="Arial", margin=dict(l=50,r=20,t=80,b=50), height=500)
    return fig
...
def prepare_weekly_summary(combined_df, run_date=date.today()):
    ...
    weekly_df = combined_df[(combined_df[EXPECTED_COL_ENROLLED_DATE].dt.date >= start_week) & (combined_df[EXPECTED_COL_ENROLLED_DATE].dt.date <= end_week)]
...
def prepare_monthly_tracker(combined_df, run_date=date.today()):
    ...
    monthly_df = combined_df[(combined_df[EXPECTED_COL_ENROLLED_DATE].dt.date >= start_month) & (combined_df[EXPECTED_COL_ENROLLED_DATE].dt.date <= end_month)]
...
def prepare_pac_metrics(pac_df_period, run_date_for_month_str=date.today()):
    """
    Prepares PAC metrics based on the provided PAC DataFrame already filtered for a specific period.
    `pac_df_period` should be the PAC data for the desired month/period.
    """
    required_cols = [EXPECTED_COL_ENROLLED_DATE, EXPECTED_COL_STATUS]
    ...
    if pac_df_period.empty or not all(col in pac_df_period.columns for col in required_cols):
        ...
        summary_empty = pd.DataFrame({"Metric": ["Total Active Sales", "Potential Rescue", "Other Non-Active", "Stick Rate Base", "Stick Rate (%)"], "Value": [0,0,0,0,"0.0"]})
    ...
    stick_rate_base = num_active_sales + num_non_active_for_stick_rate # This is len(pac_df_period)
    stick_rate = (num_active_sales / stick_rate_base * 100) if stick_rate_base > 0 else 0.0
    
    # Diagnostic: If no active sales, print unique statuses from the period's PAC data
    if num_active_sales == 0 and not pac_df_period.empty:
        print(f"DIAGNOSTIC (prepare_pac_metrics for {month_year_str}): No active sales found in PAC data for this period.")
        print(f"  Defined ACTIVE_SALE_STATUSES: {ACTIVE_SALE_STATUSES}")
        unique_statuses_in_period = pac_df_period[EXPECTED_COL_STATUS].unique().tolist()
        print(f"  Unique statuses found in PAC data for this period: {unique_statuses_in_period}")
    ...
    # Calculate "Other Non-Active" for the summary table:
    # These are statuses that are not ACTIVE_SALE and also not POTENTIAL_RESCUE
    ...
    summary_metrics_df = pd.DataFrame({
        "Metric": [
            ...
            f"Other Non-Active (Not Active/NSF) ({month_year_str})", # Clarified label
        ...
        "Value": [
            ...
            stick_rate_base, # This is total records in pac_df_period
            f"{stick_rate:.1f}"
        ]
    })
    # The first element returned is the table of daily sales, which might be empty.
    # It's better to return the full pac_df_period if the app wants to display it, 
    # or just the daily_active_sales_table for the chart.
    # For consistency, returning daily_active_sales_table.
    return daily_active_sales_table, summary_metrics_df, fig_sales, fig_gauge


# --- Main execution block (for standalone run to generate CSV) ---
    ...
    if client:
        ...
        if not combined_df.empty:
            ...
            try:
                ...
                if PAC_RAW_SHEET_NAME in all_dfs and not all_dfs[PAC_RAW_SHEET_NAME].empty:
                    pac_df = all_dfs[PAC_RAW_SHEET_NAME]
                    pac_output_csv_path = "processed_pac_data.csv"
                    pac_df.to_csv(pac_output_csv_path, index=False)
                    print(f"Successfully saved PAC data ({len(pac_df)} rows) to '{pac_output_csv_path}'")

                    # --- Standalone Run Diagnostics for PAC Metrics ---
                    print("\n--- Running Standalone PAC Metrics Diagnostic ---")
                    # Simulate a period, e.g., current month for testing
                    current_processing_date = date.today() 
                    start_m, end_m = get_target_processing_month_dates(current_processing_date)
                    pac_df_for_period_standalone = pac_df[
                        (pac_df[EXPECTED_COL_ENROLLED_DATE].dt.date >= start_m) &
                        (pac_df[EXPECTED_COL_ENROLLED_DATE].dt.date <= end_m)
                    ]
                    if not pac_df_for_period_standalone.empty:
                        _, summary_df, _, _ = prepare_pac_metrics(pac_df_for_period_standalone, current_processing_date)
                        print("\nPAC Summary Metrics for current month (standalone test):")
                        print(summary_df)
                    else:
                        print(f"No PAC data for the current month ({start_m.strftime('%B %Y')}) in standalone test.")
